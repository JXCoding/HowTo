    ...
    public void init() {
        ...
        if(!getFilePath().exists()) {
            try {
                ...
                cfg.set("JoinMessage", "&7Hallo &b@%player%&7, wenn du das Tutorial starten willst, führe &e/course" + " §7aus. %n%Dort wirst du weitere Hilfe bekommen.");
        ...
        if(!getFilePath().exists()) {
            try {
                ...
                cfg.set("JoinMessage", "&7Hey &b@%player%&7, if you want to start the Tutorial, do &e/course" + " §7aus. %n%There you will get more informations.");
                cfg.save(getFilePath());
            } catch (IOException e) {
                e.printStackTrace();
            }
